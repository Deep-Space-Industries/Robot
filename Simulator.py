...
COLOR_FOR_BEST = [(255, 0, 10), (0, 255, 128), (0, 0, 255), (238, 114, 114),
                  (255, 185, 185)]  # Colors for coloring the individuals with highest fitness scores


# Define individual
class Individual:
    def __init__(self):
        self.nn = NeuralNetwork(13, [1], 2, tanh,
                                0.1)  # In this case, parameters mean 13 inputs, 1 hidden node, 2 output nodes, tanh as the activation funcion, and 0.1 learning rate
        self.robot = Robot(random.randint(100, 1000), random.randint(100, 1000), \
                           0, 0, 20, walls)  # Each individual is assigned a simulateed robot
        self.robot.draw()
        self.robot.draw_sensors()
        sensors = self.robot.get_sensors()  # Get sensors value from the robot
        input = [np.array([sensors])]
        input = valueScaler(input[0], 0, 200, -3, 3)  # Scale values
        ...
        input[0] = np.append(input[0], randomWeights([[0]])[
            0])  # In the begining the hidden nodes is empty, so passing a random value
        self.position = self.nn.forwardPropagation(input[0])  # Position is the actual velocities
    ...
    def update_individual(self):
        ...
        input[0] = np.append(input[0],
                             self.nn.hiddenOutputs)  # Use the hidden nodes from previous moment as the input for this moment.
        self.position = self.nn.forwardPropagation(input[0])  # Position is the actual velocities
        self.position = valueScaler(self.position[0], -1, 1, -15, 15)[0]  # Scale velocities
        ...
        self.fitness = self.fitnessFunction(velocity, sensors, self.robot.environment.cleared_dust, collision, 0.4, 0.6,
                                            0.25)  # Update fitness
        self.fitness -= math.sqrt(
            (math.sqrt(abs((min(sensors) - 200))) + (self.robot.environment.cleared_dust * 0.4))) * 0.05
        if (min(sensors) > 5):
            self.fitness += math.sqrt((30 + self.robot.environment.cleared_dust * 0.4)) * 0.1

    def fitnessFunction(self, velocity, sensor, dust, collision, w1, w2, w3):
        vels = valueScaler([velocity[0], velocity[1]], -15, 15, 0, 1)[0]
        ...
        dustScore = w2 * dust
        turnScore = w3 * (deltaVelocity * importance)
        print("VelScore", velocityScore)
        print("dustScore", dustScore)
        print("turnScore", turnScore)

        fitness = velocityScore + dustScore + turnScore

        if (deltaVelocity >= 0.5 * random.uniform(0, 1)):
            fitness += 10 + dustScore * 0.2
        print("after Delta", fitness)
...
# Perform mutation, with mutation rate 0.05
...
def updateEpoch(population):
    ...
    for o in range(len(population.offsprings)):
        ...
        population.offsprings[o].fitness = population.offsprings[o].fitnessFunction( \
            ...
            po.robot.collision1, 0.4, 0.6, 0.25)
...
while not done:
    ...
    for individual in population.individuals:
        ...
        print(
            f"fitness: {individual.fitness}. LV: {individual.robot.left_velocity}, RV: {individual.robot.right_velocity}, Po: {individual.position}")
    blit_text(f"Epoch:{n_epochs}", 100, 900, WHITE, BLACK, 36)
